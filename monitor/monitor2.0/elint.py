"""
==================================================
ELINT数据处理模块
提供数据解析、JSON文件读写、性能数据获取等功能
==================================================
"""

import os
import re
import json
import csv
import subprocess
from datetime import datetime, date
from pathlib import Path

from common import *
from find import find_files


def normalize_thread_key(cores):
    """
    标准化线程键名
    
    参数:
        cores: 线程数
    
    返回:
        str: 标准化的线程键名
    """
    try:
        return str(int(cores))
    except Exception:
        return '0'


def get_data_info(datafile):
    """
    从数据文件中解析性能信息
    
    参数:
        datafile: 数据文件路径
    
    返回:
        dict: 解析后的性能数据
    """
    daily_metric = {}
    normalized_path = datafile.replace('\\', '/')
    cores = re.findall(r'thread_([0-9]+)', normalized_path)
    if len(cores) == 0:
        cores = 0
    else:
        cores = cores[0]
    thread_key = normalize_thread_key(cores)

    with open(datafile, "r", encoding="utf-8") as f:
        for line in f:
            if "#" in line or line == "\n" or line == "":
                continue
            # 正则匹配数据行：dict set <ID> <stage> {<runtime> ... <memory> ...}
            datas = re.findall(r'dict set\s(\d+)\s+([^\s]+)\s+\{([0-9.]*)\s+[0-9.]+\s+([0-9.]*).*', line.strip())
            if not datas:
                continue

            data = datas[0][0]          # 数据ID
            stage = datas[0][1]         # 阶段名称
            runtime = float("{:.2f}".format(float(datas[0][2])))
            memory = float("{:.2f}".format(float(datas[0][3])))

            if data not in daily_metric:
                daily_metric[data] = {}
            if stage not in daily_metric[data]:
                daily_metric[data][stage] = {'thread_metrics': {}}
            if 'thread_metrics' not in daily_metric[data][stage]:
                daily_metric[data][stage] = {'thread_metrics': {}}

            daily_metric[data][stage]['thread_metrics'][thread_key] = {
                'runtime': runtime,
                'memory': memory,
                'cores': int(thread_key)
            }

    return daily_metric


def save_json(json_path, data):
    """
    保存数据到JSON文件
    
    参数:
        json_path: JSON文件路径
        data: 要保存的数据
    """
    log("保存新的 json 文件中...")
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)


def merge_stage_info(existing_stage, new_stage):
    """
    合并阶段信息，支持多线程数据
    
    参数:
        existing_stage: 已有的阶段数据
        new_stage: 新的阶段数据
    
    返回:
        dict: 合并后的阶段数据
    """
    if existing_stage is None:
        return new_stage

    # 转换旧格式到新格式
    if 'thread_metrics' not in existing_stage:
        existing_thread = normalize_thread_key(existing_stage.get('cores', 0))
        existing_stage = {
            'thread_metrics': {
                existing_thread: {
                    'runtime': existing_stage.get('runtime'),
                    'memory': existing_stage.get('memory'),
                    'cores': int(existing_thread)
                }
            }
        }

    # 合并线程数据
    if 'thread_metrics' in new_stage:
        for thread_key, thread_info in new_stage['thread_metrics'].items():
            existing_stage['thread_metrics'][thread_key] = thread_info
    else:
        thread_key = normalize_thread_key(new_stage.get('cores', 0))
        existing_stage['thread_metrics'][thread_key] = {
            'runtime': new_stage.get('runtime'),
            'memory': new_stage.get('memory'),
            'cores': int(thread_key)
        }

    return existing_stage


def get_data_from(path, depth, pattern, name):
    """
    从目录中获取所有项目数据
    
    参数:
        path: 根目录路径
        depth: 搜索深度
        pattern: 文件匹配模式
        name: 路径模式列表
    
    返回:
        dict: 所有项目的性能数据
    """
    log(f"正在获取数据...")
    projects_data = {}
    
    for txt in find_files(root_dir=path, max_depth=depth, target_pattern=pattern, path_patterns=name):
        normalized_txt = txt.replace('\\', '/')
        casename = re.findall(r'/[0-9]+_(.*)\.txt', normalized_txt)
        casename = casename[0] if casename else Path(txt).stem
        
        if casename not in projects_data:
            projects_data[casename] = {
                'project_name': casename,
                'description': 'qor case 单线程监控,非FOM',
                'daily_metrics': {}
            }

        new_data = get_data_info(txt)
        for date_str, stage_info in new_data.items():
            if date_str not in projects_data[casename]['daily_metrics']:
                projects_data[casename]['daily_metrics'][date_str] = {}
            for stage, stage_data in stage_info.items():
                existing_stage = projects_data[casename]['daily_metrics'][date_str].get(stage)
                projects_data[casename]['daily_metrics'][date_str][stage] = merge_stage_info(existing_stage, stage_data)

    return projects_data


def get_target_items(root_dir: str, item_type: str, level: int = 1):
    """
    获取指定目录下指定类型的项目
    
    参数:
        root_dir: 根目录路径
        item_type: 类型 (folder/file)
        level: 目录层次（仅支持1）
    
    返回:
        list: 匹配的路径列表
    """
    if level != 1:
        return []
    
    target_path = Path(root_dir)
    if not target_path.exists() or not target_path.is_dir():
        return []
    
    result = []
    for item in target_path.iterdir():
        if item_type == "folder" and item.is_dir():
            result.append(str(item.absolute()))
        elif item_type == "file" and item.is_file():
            result.append(str(item.absolute()))
    
    return result





def add_json(new_json, old_json, date_str, thread, original_path):
    """
    将新数据合并到JSON中
    
    参数:
        new_json: 新JSON数据
        old_json: 旧JSON数据
        date_str: 日期字符串
        thread: 线程标识
        original_path: 原始数据路径
    
    返回:
        dict: 合并后的JSON数据
    """
    for case in old_json:
        if case in new_json:
            # 项目已存在，更新日期数据
            if date_str in old_json[case]["daily_metrics"]:
                daily_metrics = old_json[case]["daily_metrics"][date_str]
                new_json[case]["daily_metrics"][date_str] = daily_metrics
            else:
                log(f"case: {case} 在日期：{date_str} 可能需要重新获取")
                new_date_data = get_data_from(original_path, 1, "*.txt", ["performance*"])
                if new_date_data == {} or case not in new_date_data:
                    log(f"case: {case} 在日期：{date_str} 不存在数据")
                else:
                    log(f"case: {case} 在日期：{date_str} 更新成功")
                    new_json[case]["daily_metrics"][date_str] = new_date_data[case]["daily_metrics"][date_str]
        else:
            # 新项目，添加数据
            if date_str in old_json[case]["daily_metrics"]:
                new_json[case] = {
                    "project_name": case,
                    "description": f"qor case {thread}监控,非FOM",
                    "daily_metrics": {
                        date_str: old_json[case]["daily_metrics"][date_str]
                    }
                }
            else:
                log(f"在旧的数据文件中不存在,case: {case} 日期: {date_str}")
    
    return new_json


def get_json_data(tool, original_path, data_path):
    """
    获取JSON数据（主入口函数）
    
    参数:
        tool: 工具名称
        original_path: 原始数据路径
        data_path: JSON保存路径
    
    返回:
        dict: 项目性能数据
    """
    try:
        thread = Path(data_path).name
        log(f"获取html需要的数据中\n\ttool:{tool}\n\tthread:{thread}")
        
        total_json_file = Path(data_path)
        new_total_json = {}
        
        # 如果JSON文件不存在，直接获取数据并保存
        if not total_json_file.exists():
            new_total_json = get_data_from(original_path, 2, "*.txt", [f"{date.today().year}" + "*", "performance*"])
            save_json(total_json_file, new_total_json)
            return new_total_json
        
        # 加载现有JSON文件
        total_json = load_json(total_json_file)
        
        # 获取原始数据文件夹列表
        original_files = get_target_items(original_path, "folder")
        
        # 收集可用日期
        available_dates = []
        for f in original_files:
            try:
                date_str = Path(f).name
                available_dates.append(datetime.strptime(date_str, "%Y-%m-%d-%H").strftime("%Y%m%d"))
            except Exception:
                continue
        available_dates = sorted(set(available_dates))
        
        # 如果没有原始数据，直接返回
        if len(original_files) == 0:
            for case_data in total_json.values():
                case_data['available_dates'] = sorted(set(total_json.get(next(iter(total_json)), {}).get('daily_metrics', {}).keys())) if total_json else []
            save_json(total_json_file, total_json)
            return total_json
        
        # 处理每个原始数据文件夹
        for f in original_files:
            print(f"正在处理文件: {f} ...")
            try:
                date_str = Path(f).name
                date_str = datetime.strptime(date_str, "%Y-%m-%d-%H").strftime("%Y%m%d")
            except Exception:
                continue
            
            # 将对应日期的数据添加到新的json中
            new_total_json = add_json(new_total_json, total_json, date_str, thread, f)
        
        # 检查是否有遗漏的case
        new_total_json = get_txt_name(original_path, new_total_json)
        
        # 为所有case补充available_dates信息
        for case_data in new_total_json.values():
            case_data['available_dates'] = available_dates if available_dates else sorted(set(case_data.get('daily_metrics', {}).keys()))
        
        # 判断是否需要保存
        if new_total_json == total_json:
            log("没有数据需要更新")
            return new_total_json
        else:
            log("数据更新并重新保存")
            save_json(total_json_file, new_total_json)
            return new_total_json
            
    except Exception as e:
        log(f"获取数据发生异常: {e}")
        return {}


def get_txt_name(path, new_json):
    """
    获取遗漏的case的txt文件
    
    参数:
        path: 搜索路径
        new_json: 当前JSON数据
    
    返回:
        dict: 补充后的JSON数据
    """
    all_txt = find_files(root_dir=path, max_depth=2, target_pattern="*.txt", 
                         path_patterns=[f"{date.today().year}" + "*", "performance*"])
    if all_txt == "{}":
        return new_json

    cases = {}
    for txt in all_txt:
        casename = re.findall(r'[0-9]+_([^\s]+).txt', txt)[0]
        cases[casename] = txt

    casenames = cases.keys()
    new = new_json.keys()
    diff1 = set(casenames) - set(new)

    log(f"case: {diff1} 需要重新获取数据 ...")
    for case in diff1:
        log(f"更新case:{case} ...")
        new_json_case = get_data_from(path, 2, f"*_{case}.txt", [f"{date.today().year}" + "*", "performance*"])
        new_json[case] = new_json_case[case]

    return new_json


def git_pull():
    """
    执行git pull拉取最新数据（已注释，暂不使用）
    """
    try:
        result = subprocess.run(
            ['git', 'pull', 'origin', 'develop'],
            cwd='/home/xbzhong/Desktop/python/monitor/rd_perf',
            capture_output=True,
            text=True,
            check=False
        )
        
        if result.returncode == 0:
            print("✅ 拉取成功")
            print(result.stdout)
        else:
            print("❌ 拉取失败")
            print(result.stderr)
            
        return result
        
    except Exception as e:
        print(f"执行异常: {e}")
        return None


def read_csv(path):
    """
    读取CSV文件，返回日期到评论的映射
    
    参数:
        path: CSV文件路径
    
    返回:
        dict: {日期: 评论}
    """
    data = {}
    with open(path, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            time = row['Date']
            time = datetime.strptime(time, "%Y-%m-%d").strftime("%Y%m%d")
            data[time] = row['comment']
    
    return data


def get_perf(mem, cpu):
    """
    获取性能数据（MR更新信息）
    
    参数:
        mem: 内存性能CSV文件路径
        cpu: CPU性能CSV文件路径
    
    返回:
        dict: 性能数据字典
    """
    try:
        mem_data = read_csv(Path(mem)) if mem else {}
        cpu_data = read_csv(Path(cpu)) if cpu else {}
        perf = {
            "mem": mem_data,
            "cpu": cpu_data
        }
        print(perf)
        return perf
    except Exception as e:
        print(f"执行异常: {e}")
        return {
            "mem": {},
            "cpu": {}
        }