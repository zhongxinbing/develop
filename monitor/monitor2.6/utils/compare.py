...
class DataComparer:
    ...
    def compare(self,
        ...
        if is_multi_dimension:
            ...
            # 多线程对比
            self.logger.info(f"对比线程: {thread_list}")
            ...
            self.logger.info(f"对比结果: {result}")
        else:
            # 单线程版本对比
    ...
    def _get_value_for_date(self, case_data: Dict, dimension: str, rule: str, 
        ...
        date_index_map = getattr(self, '_date_index_cache', None)
        cache_key = (id(dates), date)
        if date_index_map is None:
            date_index_map = {}
            self._date_index_cache = date_index_map
        
        if cache_key not in date_index_map:
            idx_map = {d: i for i, d in enumerate(dates)}
            date_index_map[cache_key] = idx_map.get(date)
        
        idx = date_index_map[cache_key]
