...
async function init() {
    ...
    if (customCaseSelect && customCaseSelect.value) {
        await loadCustomDimensions(customCaseSelect.value);
    } else if (currentProjectId) {
        if (customCaseSelect) customCaseSelect.value = currentProjectId;
        await loadCustomDimensions(currentProjectId);
    }
}
...
function initCustomChart() {
    const chartDom = document.getElementById('custom-chart');
    if (chartDom) {
        if (customChart) {
            customChart.dispose();
        }
        customChart = echarts.init(chartDom);
    }
}
 ...
 * 加载项目的维度信息
 */
async function loadCustomDimensions(projectId) {
    if (!projectId) return;
    
    try {
        const response = await fetch(`/api/dimensions/${projectId}`);
        const data = await response.json();
        
        if (data.success && data.dimensions) {
            customDimensions = data.dimensions;
            
            // 更新规则选择器
            const ruleSelect = document.getElementById('customRuleSelect');
            if (ruleSelect && customDimensions.rules) {
                const currentValue = ruleSelect.value;
                ruleSelect.innerHTML = '<option value="">-- 请选择阶段 --</option>' + 
                    customDimensions.rules.map(rule => `<option value="${rule}">${rule}</option>`).join('');
                
                if (currentValue && customDimensions.rules.includes(currentValue)) {
                    ruleSelect.value = currentValue;
                    customCurrentConfig.rule_name = currentValue;
                }
            }
            
            // 更新Y轴线程选择器
            updateCustomYThreadSelect();
            
            // 更新X轴线程选择器
            updateCustomXThreadSelect();
            
            // 更新日期范围输入框的placeholder
            if (customDimensions.date_range) {
                const dateStart = document.getElementById('customDateStart');
                const dateEnd = document.getElementById('customDateEnd');
                if (dateStart) dateStart.placeholder = customDimensions.date_range.start || '开始日期';
                if (dateEnd) dateEnd.placeholder = customDimensions.date_range.end || '结束日期';
            }
            
            // 更新配置预览
            updateCustomConfigPreview();
        }
    } catch (error) {
        console.error('加载维度信息失败:', error);
        showNotification('加载维度信息失败', true);
    }
}
 ...
 * 更新Y轴线程选择器
 */
function updateCustomYThreadSelect() {
    const threadSelect = document.getElementById('customYThread');
    if (!threadSelect || !customDimensions) return;
    
    const threads = customDimensions.threads || ['0'];
    const currentValue = threadSelect.value;
    
    threadSelect.innerHTML = threads.map(thread => {
        const displayName = thread === '0' ? '线程0 (默认)' : `线程 ${thread}`;
        return `<option value="${thread}">${displayName}</option>`;
    }).join('');
    
    if (currentValue && threads.includes(currentValue)) {
        threadSelect.value = currentValue;
        customCurrentConfig.y_thread = currentValue;
    } else if (threads.length > 0) {
        threadSelect.value = threads[0];
        customCurrentConfig.y_thread = threads[0];
    }
}
 ...
 * 更新X轴线程选择器
 */
function updateCustomXThreadSelect() {
    const threadSelect = document.getElementById('customXThreadSelect');
    if (!threadSelect || !customDimensions) return;
 ...
 * 更新X轴相关UI
 */
function updateCustomXAxisUI() {
    const xType = document.getElementById('customXType').value;
    const xDimensionGroup = document.getElementById('customXDimensionGroup');
    const dateRangeInputs = document.getElementById('customDateRangeInputs');
    const xThreadSelect = document.getElementById('customXThreadSelect');
    const xDimensionLabel = document.getElementById('customXDimensionLabel');
    
    if (xType === 'date') {
        dateRangeInputs.style.display = 'flex';
        xThreadSelect.style.display = 'none';
        xDimensionLabel.textContent = '📅 日期范围';
    } else {
        dateRangeInputs.style.display = 'none';
        xThreadSelect.style.display = 'block';
        xDimensionLabel.textContent = '🔢 X轴线程';
    }
}
 ...
 * 更新配置预览
 */
function updateCustomConfigPreview() {
    const preview = document.getElementById('customConfigPreview');
    if (!preview) return;
    
    const xType = customCurrentConfig.x_type === 'date' ? '日期' : '线程数';
    const yMetricMap = {
        'runtime': 'Runtime (秒)',
        'memory': 'Memory (MB)',
        'cores': '核心数'
    ...
    let xDetail = '';
    if (customCurrentConfig.x_type === 'date') {
        xDetail = customCurrentConfig.date_start && customCurrentConfig.date_end ? 
            `${customCurrentConfig.date_start} → ${customCurrentConfig.date_end}` : '全部日期';
    } else {
        xDetail = `线程 ${customCurrentConfig.x_dimension || '0'}`;
    }
    
    preview.innerHTML = `
        📊 配置: X轴 = ${xType} (${xDetail}) | 
        Y轴 = ${yMetricMap[customCurrentConfig.y_metric]} (线程 ${customCurrentConfig.y_thread})
        ${customCurrentConfig.rule_name ? ` | 阶段: ${customCurrentConfig.rule_name}` : ''}
    `;
}
...
function getCustomConfigFromForm() {
    const projectId = document.getElementById('customCaseSelect').value;
    const ruleName = document.getElementById('customRuleSelect').value;
    const xType = document.getElementById('customXType').value;
    const yMetric = document.getElementById('customYMetric').value;
    const yThread = document.getElementById('customYThread').value;
    ...
    if (xType === 'date') {
        dateStart = document.getElementById('customDateStart').value;
        dateEnd = document.getElementById('customDateEnd').value;
    } else {
        xDimension = document.getElementById('customXThreadSelect').value;
    }
...
async function applyCustomConfig() {
    ...
    if (!config.rule_name) {
        showNotification('请选择阶段', true);
        return;
    }
    
    // 更新当前配置
    customCurrentConfig = { ...config };
    updateCustomConfigPreview();
    
    // 显示用户配置指示器
    const badge = document.getElementById('userConfigBadge');
    if (badge) badge.style.display = 'inline-flex';
    
    // 显示加载状态
    const statusEl = document.getElementById('customChartStatus');
    if (statusEl) {
        statusEl.className = 'config-status info';
        statusEl.style.display = 'block';
        statusEl.innerHTML = '🔄 正在加载数据...';
    }
    
    try {
        const response = await fetch('/api/custom_chart_data', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...
                x_axis: {
                    type: config.x_type,
                    dimension: config.x_dimension
                },
                y_axis: {
                    metric: config.y_metric,
                    thread: config.y_thread
                },
                date_range: {
        ...
        if (result.success) {
            ...
            if (statusEl) {
                ...
                statusEl.innerHTML = `✅ 图表已更新 | 数据点: ${result.x_data.length} | ${result.date_range_used?.start || ''} → ${result.date_range_used?.end || ''}`;
                setTimeout(() => {
            ...
            if (titleEl) {
                const metricName = config.y_metric === 'runtime' ? 'Runtime' : (config.y_metric === 'memory' ? 'Memory' : '核心数');
                const xName = config.x_type === 'date' ? '日期' : '线程数';
                titleEl.innerHTML = `🎨 自定义图表 - ${metricName} vs ${xName}`;
            }
        } else {
            throw new Error(result.error || '获取数据失败');
        }
    } catch (error) {
        console.error('加载自定义图表失败:', error);
        if (statusEl) {
            statusEl.className = 'config-status error';
            statusEl.innerHTML = `❌ 加载失败: ${error.message}`;
            statusEl.style.display = 'block';
            setTimeout(() => {
                if (statusEl) statusEl.style.display = 'none';
            }, 5000);
        }
        showNotification('加载图表数据失败: ' + error.message, true);
    }
}
...
function renderCustomChart(data) {
    ...
    const isRuntime = data.y_metric === 'runtime';
    const isMemory = data.y_metric === 'memory';
    
    const yAxisName = isRuntime ? 'Runtime (秒)' : (isMemory ? 'Memory (MB)' : '核心数');
    const color = isRuntime ? '#6366f1' : (isMemory ? '#10b981' : '#f59e0b');
    const unit = isRuntime ? '秒' : (isMemory ? 'MB' : '核心');
    
    // 处理Y轴数据
    const validData = yData.filter(v => v !== null && v !== undefined);
    const avgValue = validData.length > 0 ? (validData.reduce((a, b) => a + b, 0) / validData.length).toFixed(2) : 0;
    
    // 判断X轴类型
    const isDateX = data.x_type === 'date';
    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: function(params) {
                ...
                const displayValue = value !== null && value !== undefined ? 
                    (isRuntime ? value.toFixed(2) : (isMemory && value >= 1024 ? (value / 1024).toFixed(2) + ' GB' : value.toFixed(2))) : 'N/A';
                return `<strong>${params[0].axisValue}</strong><br/>${params[0].seriesName}: ${displayValue} ${unit}`;
            }
        },
        grid: {
            left: '8%',
        ...
        xAxis: {
            type: isDateX ? 'category' : 'value',
            name: isDateX ? '日期' : '线程数',
            nameTextStyle: { color: '#cbd5e1', fontSize: 12 },
            data: isDateX ? xData : undefined,
            axisLabel: {
                color: '#94a3b8',
                fontSize: 11,
                rotate: isDateX && xData.length > 10 ? 30 : 0
            },
            axisLine: { lineStyle: { color: '#475569' } },
            splitLine: isDateX ? { show: false } : { lineStyle: { color: 'rgba(71, 85, 105, 0.3)' } }
        },
        yAxis: {
            type: 'value',
            name: yAxisName,
            ...
            axisLabel: {
                ...
                fontSize: 11,
                formatter: (value) => {
                    if (isMemory && value >= 1024) return (value / 1024).toFixed(1) + ' GB';
                    if (isRuntime) return value.toFixed(2);
                    return value.toFixed(0);
                }
            },
        ...
        series: [{
            name: data.y_metric === 'runtime' ? 'Runtime' : (data.y_metric === 'memory' ? 'Memory' : '核心数'),
            type: 'line',
            data: yData,
            smooth: true,
            lineStyle: { width: 3, color: color },
            ...
            markLine: avgValue > 0 ? {
                ...
                label: { formatter: `平均值: ${avgValue} ${unit}`, color: '#f59e0b' }
            } : undefined
        }],
        toolbox: {
            feature: {
    ...
    customChart.setOption(option, { notMerge: true });
    setTimeout(() => customChart.resize(), 100);
}
...
function resetCustomConfig() {
    ...
    const ruleSelect = document.getElementById('customRuleSelect');
    const xTypeSelect = document.getElementById('customXType');
    const yMetricSelect = document.getElementById('customYMetric');
    const yThreadSelect = document.getElementById('customYThread');
    const dateStart = document.getElementById('customDateStart');
    const dateEnd = document.getElementById('customDateEnd');
    const xThreadSelect = document.getElementById('customXThreadSelect');
    ...
    if (caseSelect && caseSelect.options.length > 0) {
        ...
        loadCustomDimensions(caseSelect.value);
    }
    
    if (ruleSelect) ruleSelect.value = '';
    if (xTypeSelect) xTypeSelect.value = 'date';
    if (yMetricSelect) yMetricSelect.value = 'runtime';
    if (yThreadSelect) yThreadSelect.value = '0';
    if (dateStart) dateStart.value = '';
    if (dateEnd) dateEnd.value = '';
    if (xThreadSelect) xThreadSelect.value = '0';
    ...
    const titleEl = document.getElementById('customChartTitle');
    if (titleEl) titleEl.innerHTML = '🎨 自定义图表';
    
    const preview = document.getElementById('customConfigPreview');
    if (preview) preview.innerHTML = '配置已重置，请选择项目和阶段后点击"生成图表"';
...
function exportCustomChart() {
    if (!customChart) {
        showNotification('没有可导出的图表', true);
        return;
    }
    
    const url = customChart.getDataURL({
        type: 'png',
        pixelRatio: 2,
    ...
    const link = document.createElement('a');
    link.download = `custom_chart_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.png`;
    link.href = url;
    link.click();
...
function bindCustomChartEvents() {
    ...
    if (caseSelect) {
        caseSelect.addEventListener('change', async (e) => {
            ...
            if (projectId) {
                await loadCustomDimensions(projectId);
            }
        });
    }
    ...
    const xTypeSelect = document.getElementById('customXType');
    if (xTypeSelect) {
        xTypeSelect.addEventListener('change', updateCustomXAxisUI);
    }
    
    const yMetricSelect = document.getElementById('customYMetric');
    if (yMetricSelect) {
        yMetricSelect.addEventListener('change', updateCustomConfigPreview);
    }
    
    const yThreadSelect = document.getElementById('customYThread');
    if (yThreadSelect) {
        yThreadSelect.addEventListener('change', updateCustomConfigPreview);
    }
    
    const applyBtn = document.getElementById('applyCustomConfigBtn');
    if (applyBtn) {
        applyBtn.addEventListener('click', applyCustomConfig);
    }
    
    const resetBtn = document.getElementById('resetCustomConfigBtn');
    if (resetBtn) {
        resetBtn.addEventListener('click', resetCustomConfig);
    }
    
    const exportBtn = document.getElementById('exportCustomChartBtn');
    if (exportBtn) {
        exportBtn.addEventListener('click', exportCustomChart);
    }
}
...
// initCustomChart();
// bindCustomChartEvents();
// loadCustomDimensions(currentProjectId || (caseSelect?.value));
