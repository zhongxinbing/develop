...
class DataComparator:
    ...
    def compare_all_rules(
        ...
        for rule in all_rules:
            ...
            if has_data:
                ...
                runtime_status = self._get_status(runtime_change_pct if tolerance_mode == 'percent' else runtime_diff, tolerance_runtime)
                ...
                if memory1 is not None and memory2 is not None:
                    ...
                    memory_status = self._get_status(memory_change_pct if tolerance_mode == 'percent' else memory_diff, tolerance_memory)
                
                runtime_significant = abs(runtime_change_pct) > tolerance_runtime if tolerance_mode == 'percent' else abs(runtime_diff) > tolerance_runtime
                memory_significant = False
                if memory_change_pct is not None:
                    memory_significant = abs(memory_change_pct) > tolerance_memory if tolerance_mode == 'percent' else abs(memory_diff) > tolerance_memory
                ...
                # Memory统计
                if memory_change_pct is not None and memory_significant:
                    if memory_diff > 0:
    ...
    def compare_single_rule(
        ...
        for i in range(min_len):
            ...
            comparison = {
                ...
                "memory_diff": round(memory_diff, 2) if memory_diff is not None else None,
                "memory_change_pct": round(memory_change_pct, 2) if memory_change_pct is not None else None,
                "memory_status": self._get_status(memory_change_pct if tolerance_mode == 'percent' else memory_diff, tolerance_memory) if memory_diff is not None else "N/A",
                "memory_significant": memory_significant
            }
    ...
    def compare_data(
        ...
        if rule_name == "all" or rule_name is None:
            return self.compare_all_rules(data1, data2, project_id, tolerance_runtime, tolerance_memory, tolerance_mode=tolerance_mode)
        else:
            return self.compare_single_rule(data1, data2, project_id, rule_name, tolerance_runtime, tolerance_memory, tolerance_mode=tolerance_mode)
