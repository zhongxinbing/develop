...
def get_available_dates_from_path(original_path):
    folders = get_target_items(original_path, "folder")
    dates = []
    for f in folders:
        try:
            date_key = datetime.strptime(Path(f).name, "%Y-%m-%d-%H").strftime("%Y%m%d")
...
def get_json_data(tool,original_path,data_path):
    ...
    new_total_json={}
    available_dates = get_available_dates_from_path(original_path)
    ...
    if not total_json_file.exists():
        ...
        for case in new_total_json:
            new_total_json[case]["available_dates"] = available_dates
    ...
    if len(original_files) == 0:
        for case in total_json:
            total_json[case]["available_dates"] = available_dates or sorted(total_json[case].get("daily_metrics", {}).keys())
    ...
    # 为每个case补充QOR可用日期元数据
    for case in new_total_json:
        new_total_json[case]["available_dates"] = available_dates or sorted(new_total_json[case].get("daily_metrics", {}).keys())
